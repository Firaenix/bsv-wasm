import {Transaction, SigHash, PrivateKey, Script, ECDSA, Hash, SigningHash, PublicKey} from '../../../packages/bsv-wasm/pkg/node/bsv_wasm';
...
describe("SigHash Tests", function() {
  it('SIGHASH_SINGLE Signed Tx matches BSV.JS', () => {
    const wasm_private_key = PrivateKey.from_wif("L31JUXCGspUREe9Gya8F2WWjeoRz3bb8AQzJjAP8ntGYp37oYdSx");
    const wasm_script = Script.from_asm_string("OP_0 OP_RETURN");
    const wasm_tx = Transaction.from_hex("");

    const js_private_key = PrivKey.fromWif(wasm_private_key.to_wif(true));
    const js_tx = Tx.fromHex(wasm_tx.to_hex());
    const js_script = JSScript.fromAsmString(wasm_script.to_asm_string());

    assert.equal(wasm_private_key.sign_message(Buffer.from("Hello, Bitcoin.")).to_der_hex(),  Ecdsa.sign(Buffer.from(Hash.sha_256(Buffer.from("Hello, Bitcoin.")).to_bytes()), KeyPair.fromPrivKey(js_private_key)).toHex() );
    
    assert.equal(js_tx.id(), wasm_tx.get_id_hex(), "Transaction IDs dont match");
    
    assert.equal(js_tx.toHex(), wasm_tx.to_hex());
    assert.equal(js_private_key.toWif(), wasm_private_key.to_wif(true));
    assert.equal(js_script.toAsmString(), wasm_script.to_asm_string());
    ...
    let wasm_sighash_u8 = wasm_tx.sighash_preimage(SigHash.SINGLE | SigHash.FORKID, 0, wasm_script, BigInt(0))
    ...
    assert.equal(Hash.sha_256d(wasm_sighash).to_hex(), JSHash.sha256Sha256(js_sighash).toString('hex'), "SHA256d hex doesnt  match")
    ...
    assert.equal(wasm_sig.to_hex(), js_sig.toHex(), "Signed sighash buffers dont match")
  ...
  it('Tests signing with a custom K and key recovery', () => {
    ...
    const wasm_private_key = PrivateKey.from_wif("L31JUXCGspUREe9Gya8F2WWjeoRz3bb8AQzJjAP8ntGYp37oYdSx");
    let recovery_sig = ECDSA.sign_with_k(wasm_private_key, wasm_private_key, msg, SigningHash.Sha256d);
    let recovered_priv_key = ECDSA.private_key_from_signature_k(recovery_sig, PublicKey.from_private_key(wasm_private_key), wasm_private_key, msg, SigningHash.Sha256d);
    assert.equal(recovered_priv_key.to_wif(), wasm_private_key.to_wif());
  });
  ...
  it('SIGHASH_ALL Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.InputsOutputs, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_NONE Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.Inputs, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_SINGLE (InputsOutput) Signed Tx matches BSV.JS ', () => {
    ...
    let js_sighash = Buffer.from(js_tx.sighashPreimage(Sig.SIGHASH_SINGLE | Sig.SIGHASH_FORKID, 0, js_script, Bn(0), Tx.SCRIPT_ENABLE_SIGHASH_FORKID));
    let wasm_sighash_u8 = wasm_tx.sighash_preimage(SigHash.InputsOutput, 0, wasm_script, BigInt(0))
    let wasm_sighash = Buffer.from(wasm_sighash_u8);
  ...
  it('SIGHASH_SINGLE | ANYONECANPAY (InputOutput) Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.InputOutput, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_ALL | ANYONECANPAY (InputOutputs) Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.InputOutputs, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_NONE | ANYONECANPAY (Input) Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.Input, 0, wasm_script, BigInt(0));
...
describe("Old Sighash functions", function() {
  it('SIGHASH_NONE NO FORK_ID Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.NONE, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_ALL NO FORK_ID Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.ALL, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_SINGLE NO FORK_ID Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.SINGLE, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_NONE | ANYONECANPAY NO FORK_ID Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.NONE | SigHash.ANYONECANPAY, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_ALL | ANYONECANPAY NO FORK_ID Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.ALL | SigHash.ANYONECANPAY, 0, wasm_script, BigInt(0));
  ...
  it('SIGHASH_SINGLE | ANYONECANPAY NO FORK_ID Signed Tx matches BSV.JS', () => {
    ...
    let wasm_sighash = wasm_tx.sighash_preimage(SigHash.SINGLE | SigHash.ANYONECANPAY, 0, wasm_script, BigInt(0));
