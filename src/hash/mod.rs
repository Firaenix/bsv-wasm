use bitcoin_hashes::{Hash as BitcoinHash,
                     ...
                     sha512};
use pbkdf2::{
  password_hash::{PasswordHash, PasswordHasher, PasswordVerifier, SaltString, Salt, Output},
  Algorithm,
  Params,
  Pbkdf2,
};
...
#[wasm_bindgen(js_name = sha256d)]
    pub fn sha_256d(input : &[u8])->Self { Hash(bitcoin_hashes::sha256d::Hash::hash(input).to_vec()) }

#[wasm_bindgen(js_name = sha256)]
    pub fn sha_256(input : &[u8])->Self { Hash(bitcoin_hashes::sha256::Hash::hash(input).to_vec()) }

#[wasm_bindgen(js_name = sha1)]
    pub fn sha_1(input : &[u8])->Self { Hash(bitcoin_hashes::sha1::Hash::hash(input).to_vec()) }

#[wasm_bindgen(js_name = ripemd160)]
    pub fn ripemd_160(input : &[u8])->Self { Hash(bitcoin_hashes::ripemd160::Hash::hash(input).to_vec()) }

#[wasm_bindgen(js_name = hash160)]
    pub fn hash_160(input : &[u8])->Self { Hash(bitcoin_hashes::hash160::Hash::hash(input).to_vec()) }

#[wasm_bindgen(js_name = sha512)]
    pub fn sha_512(input : &[u8])->Self { Hash(bitcoin_hashes::sha512::Hash::hash(input).to_vec()) }

#[wasm_bindgen(js_name = pbkdf2Sha256)]
    pub fn pbkdf2_sha256(password : &[u8], salt : &[u8], rounds : u32)-> Self {
        let saltString = &std::str::from_utf8(salt).unwrap().to_string();
        let s      = Salt::new(saltString).unwrap();
        let params = Params{rounds : rounds, output_length : 32};
        let hash = Pbkdf2.hash_password(password, None, params, s).unwrap().hash.unwrap();
        let result: Vec<u8> = hash.as_bytes().iter().cloned().collect();
