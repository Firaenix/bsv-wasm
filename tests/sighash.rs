...
mod sighash_tests {
    ...
    fn sighash_inputs_output_single() {
        ...
        let sig = tx.sign(&priv_key, sighash, 0, &signing_script, 0, None).unwrap();
        assert_eq!(
            sig.to_hex().unwrap(),
        ...
        assert!(tx.verify(&PublicKey::from_private_key(&priv_key), &sig, None));
    }
    ...
    fn sighash_none_anyonecanpay_no_fork_id() {
        ...
        let sig = tx.sign(&priv_key, SigHash::try_from(sighash).unwrap(), 0, &signing_script, 0, None).unwrap();
        assert_eq!(
            sig.to_hex().unwrap(),
