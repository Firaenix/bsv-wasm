...
mod interpreter_signature_tests {
    ...
    fn checksig_with_codeseparator_test() {
        ...
        let sig1 = tx.sign(&private_key, SigHash::InputsOutputs, 0, &first_locking_script, 0, true).unwrap();
        let sig2 = tx.sign(&private_key, SigHash::InputsOutputs, 0, &second_locking_script, 0, true).unwrap();
        let sig3 = tx.sign(&private_key, SigHash::InputsOutputs, 0, &third_locking_script, 0, true).unwrap();
    ...
    fn multisig_equal_amount_pubkeys_test() {
        ...
        let sig1 = tx.sign(&pk1, SigHash::ALL, 0, &locking_script, 0, true).unwrap();
        let sig2 = tx.sign(&pk2, SigHash::ALL, 0, &locking_script, 0, true).unwrap();
        let sig3 = tx.sign(&pk3, SigHash::ALL, 0, &locking_script, 0, true).unwrap();
