...
mod script_tests {
    ...
    fn from_problematic_asm_string() {
        ...
        assert_eq!(script.to_asm_string(), "OP_RETURN non-script-data:03026d02060568656c6c6f");
    }
    ...
    fn from_asm_string_custom_script() {
        ...
        assert_eq!(
            script.to_asm_string(),
            format!("OP_HASH160 {} OP_EQUALVERIFY {} OP_RETURN non-script-data:4d0001{}", hash_hex, p2pkh_locking_hex, meta)
        );
    }
    ...
    #[test]
    fn scrypt_with_non_script_data_malformed() {
        ...
        assert_eq!(
            ...
            &[
                ScriptBit::OpCode(OpCodes::OP_RETURN),
                ScriptBit::NonScriptData(hex::decode("00010100010001000100010001000100010001001400000000").unwrap()),
            ]
        );

        assert_eq!(&script.to_asm_string(), "OP_RETURN non-script-data:00010100010001000100010001000100010001001400000000");
        assert_eq!(&script.to_hex(), "6a00010100010001000100010001000100010001001400000000");

        let script = Script::from_asm_string(&script.to_asm_string()).unwrap();
    ...
    #[test]
    fn scrypt_with_non_script_data() {
        let script = Script::from_hex("006a0051").unwrap();
        ...
        assert_eq!(
            ...
            &[
                ScriptBit::OpCode(OpCodes::OP_0),
                ScriptBit::OpCode(OpCodes::OP_RETURN),
                ScriptBit::NonScriptData(hex::decode("0051").unwrap()),
            ]
        );

        assert_eq!(&script.to_asm_string(), "0 OP_RETURN non-script-data:0051");
        assert_eq!(&script.to_hex(), "006a0051");

        let script = Script::from_asm_string("0 OP_RETURN OP_0 OP_1").unwrap();
    ...
    fn non_script_data_asm() {
        ...
        assert_eq!(&script.to_asm_string(), "0 OP_RETURN non-script-data:0101020102");
        assert_eq!(&script.to_hex(), "006a0101020102");

        let script = Script::from_asm_string("0 OP_RETURN non-script-data:0101020102").unwrap();
