...
mod xpriv_tests {
    ...
    fn xpriv_parts_to_xpriv_string() {
        ...
        let parent_chain_code =
            hex::decode("05aae71d7c080474efaab01fa79e96f4c6cfe243237780b0df4bc36106228e31")
                .unwrap();
        ...
        let parent_fingerprint =
            &Hash::hash_160(&hex::decode(parent_public_key).unwrap()).to_bytes()[0..4];

        let key = ExtendedPrivateKey::new(
            &private_key,
            parent_chain_code.as_slice(),
            &1,
            &0,
    ...
    fn from_xprv_string_and_derive() {
        ...
        assert_eq!(
            derived_key.get_private_key().to_wif(true).unwrap(),
            "L5LxK8WV9wNDemaYBtpEURWi3sHmGsEHpSGmSfahQrreTYKukp9W"
        );
        assert_eq!(
            derived_key.get_public_key().to_hex().unwrap(),
        ...
        assert_eq!(
            second_derived.get_private_key().to_wif(true).unwrap(),
            "KwX8mbobXJQ89SzPXHJ8fLGZ7ya6GyqAMLEZ8Cs2QWSm6GSAvQVg"
        );
        assert_eq!(
            second_derived.get_public_key().to_hex().unwrap(),
            "02c815eb6b999ae9ac4edba5ca1b9a57723b4fa749afe3d3994462f0c4f2efc7dd"
        );
    }
    ...
    fn from_xprv_string_and_derive_from_path() {
        ...
        assert_eq!(
            derived_key.get_private_key().to_wif(true).unwrap(),
            "KxUAqUXuB3Ksh3QwnorUhATf2bNY6CPjD3dv8EeTXdVeQF8RYQpL"
        );
        assert_eq!(
            derived_key.get_public_key().to_hex().unwrap(),
            "03c188374826dc4a986adf53b01d1eb5ca4bf37f0c6ceea63cd6e350a56883b369"
        );
    }
    ...
    fn from_mnemonic_xprv() {
        let mnemonic =
            "vapor cabbage jacket unveil permit web live pyramid husband final plug metal";
